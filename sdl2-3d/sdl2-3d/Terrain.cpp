...
void Terrain::setTexture(Texture& texture, unsigned int height)
{
	assert (height < m_uiNumTextures && "Height larger than amount of layers");
	m_GLTextures[height] = texture.getTextureID();
}
...
bool Terrain::LoadHeightmap( HeightMap& heightMap )
{
    ...
    m_HeightmapDimensions = uivec2(width, height);
