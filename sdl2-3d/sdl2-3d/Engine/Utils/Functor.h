...
namespace Func
{
	...
	{
		return new FunctorImpl<RET, Args...>(ret, std::forward<RET(__cdecl*)(Args2...)>(f), std::forward<Args>(args)...);
	}
	template <typename... Args, typename... Args2>
	Functor* makeFunctor(void(__cdecl*f)(Args2...), Args&... args)
	{
		return new FunctorImpl<void, Args...>(std::forward<void(__cdecl*)(Args2...)>(f), std::forward<Args>(args)...);
	}
	...
	{
		return new FunctorImpl<RET, Args...>(ret, std::forward<RET(__stdcall*)(Args2...)>(f), std::forward<Args>(args)...);
	}
	template <typename... Args, typename... Args2>
	Functor* makeFunctor(void(__stdcall*f)(Args2...), Args&... args)
	{
		return new FunctorImpl<void, Args...>(std::forward<void(__stdcall*)(Args2...)>(f), std::forward<Args>(args)...);
	}
}